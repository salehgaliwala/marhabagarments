...
/*$sql = "SELECT
                                ...
                                INNER Join jobitems ON jobs.jobid = jobitems.jobid
                                Left Join dresses ON jobitems.dressid = dresses.dressid
...
{
    $select  = "SELECT * FROM production WHERE jobid = ".$jobid;
...
//    echo  $row_cnt.'row';
    ...
    if(!$row_cnt)
    {
        $insert = 'INSERT INTO production (prodnum ,jobid ) VALUES( "'.$prodnum.'","'.$jobid.'" )';
        $flag = mysqli_query($con, $insert);
    }
}

if($flag) {
  $update =   "UPDATE jobs set status = 'Processing' WHERE  jobid IN ( " . $match . ")";
