    ...
    { 
        ...
        $scope.filteredList  = _.filter($scope.allItems,
                 function(item){  
                     ...
                     return companysearchUtil(item,$scope.doText);
                 });
...
function companysearchUtil(item,toSearch)
{
    ...
    return ( item.donum.toLowerCase().indexOf(toSearch.toLowerCase()) > -1 || item.companyname.toLowerCase().indexOf(toSearch.toLowerCase()) > -1 || item.locationname.toLowerCase().indexOf(toSearch.toLowerCase()) > -1 )
                     ? true : false ;
}
